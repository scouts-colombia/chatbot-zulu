...
**Cambios de producto acordados el 2026-08-20:**
- **Consentimiento sin versión (esta rama).** El gate deja de comparar versiones de política: basta con que la persona la acepte una vez. Se eliminan `policy_version` de `consent_acceptance_events` y `privacy_policy_version_accepted` de `profiles`; el gate mira `privacy_policy_accepted_at`. **Consecuencia aceptada explícitamente:** si el texto de la política cambia, no queda forma de saber quién aceptó cuál versión.
- **La allowlist del piloto se va a quitar (pendiente).** Hoy `0012` hace que un correo en `allowed_emails` nazca `activo` y uno no invitado nazca `pendiente_autorizacion`. Cuando se quite, toda cuenta nueva nacerá `activo`. Hasta entonces NO tocar el comentario de `0001_schema_rls.sql` sobre el default de `account_status`: está desactualizado (habla del flujo de menores, derogado el 2026-08-01) pero va a volver a cambiar de significado con esta decisión.
...
- Poda de la plantilla hecha en `chore/limpieza-plantilla`: se eliminaron 12 componentes de `components/ui` sin consumidor, `hooks/use-mobile.ts`, `lib/constants.ts`, `lib/supabase/client.ts`, los estilos de los editores de artefactos (CodeMirror, ProseMirror, skeletons), la fuente Geist Mono y las dependencias `cmdk`, `sonner`, `@vercel/analytics`, `@vercel/otel` y `@opentelemetry/*` con su `instrumentation.ts` (no hay observabilidad conectada en Vercel). No hay ningún TODO/FIXME en el código.
