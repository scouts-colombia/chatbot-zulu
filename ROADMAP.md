...
- Migraciones aplicadas al proyecto: `0001`–`0013`. `0012` crea la allowlist del piloto: las cuentas invitadas nacen `activo`, las demás `pendiente_autorizacion`; no modifica cuentas existentes. Verificada en Supabase el 2026-08-06 (tabla, RLS sin políticas, funciones, default y preservación de estados). `0013` agrega a `model_request_events` el desglose nullable de prompt, File Search/herramientas, caché, respuesta visible, pensamiento y nivel, conserva los aliases históricos y añade una vista diaria por modelo/nivel; aplicada y verificada el 2026-08-07.
...
- [x] Verificar RLS con el JWT del usuario: `scripts/verify-rls.mjs` — **VERDE 25/25** el 2026-08-06 (aislamiento entre Scouts, registro invitado activo y no invitado pendiente, no auto-escalamiento de rol, mensajes de asistente no forjables, archivadas no aceptan mensajes y tablas de servidor invisibles).
