...
> Guía de diseño de la portada. Archivos que toca: `app/page.tsx`,
> `components/chat/conversacion.tsx`, `components/chat/burbuja.tsx`,
> `app/api/chat/route.ts` y `app/(auth)/formulario-auth.tsx`.
> La referencia normativa de marca es `DESIGN.md`.
...
- Autoridad visual: el repo frontend `ruta` (DNPJ), en especial `/diseno/componentes`. Reutilizar Futura Std, su fondo crema con lavados de color, los cinco colores de marca, radios, bordes, foco y superficies liquid-glass. No reintroducir el fondo morado del login.
