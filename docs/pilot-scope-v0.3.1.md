...
> 8. **Turno público antes del registro (2026-08-07).** La portada muestra directamente el chat y permite exactamente un turno de prueba antes de exigir registro o inicio de sesión. La identidad anónima de Supabase es un mecanismo interno: el visitante acepta la política antes de enviar, el límite se aplica por identidad seudónima con un tope diario complementario por red y no se almacenan IP ni user-agent crudos. Al intentar un segundo turno se conserva el borrador y se presenta la puerta de autenticación; al convertir la sesión o iniciar sesión con una cuenta existente se conserva la conversación. El turno de prueba no reduce la cuota diaria de la cuenta permanente. Esta decisión amplía la superficie de autenticación de §2.1 y la secuencia de la Definition of Done (§18), que ahora comienza con este turno público opcional.
>
